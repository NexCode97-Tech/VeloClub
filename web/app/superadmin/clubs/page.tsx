  ...
  return (
    <div className="p-8">
      ...
      <div className="space-y-3">
        {clubs.map(club => (
          <div key={club.id} className="bg-white rounded-xl border border-slate-200 px-5 py-4 flex items-center justify-between shadow-sm">
            <div>
            ...
            <div className="flex gap-2">
              <Button
                ...
                className={`cursor-pointer ${club.active ? 'text-orange-600 border-orange-300' : 'text-green-600 border-green-300'}`}
              >
                <Power className="w-4 h-4 mr-1" />
                {club.active ? 'Desactivar' : 'Activar'}
              </Button>
              <Button size="sm" variant="ghost" className="text-red-500 hover:text-red-700 cursor-pointer" onClick={() => handleDelete(club.id)}>
