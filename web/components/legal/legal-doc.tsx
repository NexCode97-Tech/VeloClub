...
function LegalContent({ markdown }: { markdown: string }) {
  ...
  lines.forEach((raw, idx) => {
    ...
    if (line.startsWith('## ')) { flushList(key); blocks.push(<h2 key={key} style={{ margin: '26px 0 10px', fontSize: 18, fontWeight: 800, color: '#381DA0' }}>{line.slice(3)}</h2>); return; }
  ...
  return (
    <div style={{ background: '#F7F7FB', minHeight: '100dvh', fontFamily: 'inherit' }}>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 20px 80px' }}>
        <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: '#381DA0', textDecoration: 'none', marginBottom: 18 }}>
          <ArrowLeft size={15} /> Volver
        </Link>

        <div style={{ background: '#fff', border: '1px solid rgba(120,80,200,0.10)', borderRadius: 20, padding: '28px 24px', boxShadow: '0 2px 20px rgba(56,29,160,0.06)' }}>
